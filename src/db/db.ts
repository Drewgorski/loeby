...
/**
 * Everything she has, as one portable file.
 *
 * There is no server, so this is the ONLY thing standing between her and losing
 * months of posture photos to a cleared Safari cache or a dead phone. Photos are
 * inlined as data URLs so a single file restores completely, with nothing to
 * re-link afterwards.
 */
export interface Backup {
  ...
  photos: (Omit<PosturePhoto, 'blob'> & { dataUrl: string })[];
}
...
export async function exportAll(): Promise<Backup> {
  const [checkins, progress, done, dayLogs, journal, photos] = await Promise.all([
    db.checkins.toArray(),
    db.progress.toArray(),
    db.done.toArray(),
    db.dayLogs.toArray(),
...
/**
 * Restore a backup, merging into whatever is already here. Rows are keyed
 * deterministically (date, or `${date}:${pose}`), so re-importing the same file
 * is a no-op rather than a pile of duplicates.
 */
export async function importAll(data: unknown): Promise<{ restored: number }> {
  const b = data as Backup;
  if (!b || b.format !== 'loeby-backup') throw new Error('That file is not a Loeby backup.');
  ...
  await db.transaction(
    'rw',
    [db.checkins, db.progress, db.done, db.dayLogs, db.journal, db.photos],
    async () => {
      if (b.checkins?.length) await db.checkins.bulkPut(b.checkins);
      if (b.progress?.length) await db.progress.bulkPut(b.progress);
      if (b.done?.length) await db.done.bulkPut(b.done);
      if (b.dayLogs?.length) await db.dayLogs.bulkPut(b.dayLogs);
